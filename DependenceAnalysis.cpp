...
bool DependenceAnalysisWrapperPass::runOnFunction(Function &F) {
  ...
  //z+
  for(BasicBlock &BB : F){
      for (BasicBlock::iterator SrcI=BB.begin();SrcI!=BB.end();SrcI++)
      {
         if (isa<StoreInst>(*SrcI) || isa<LoadInst>(*SrcI))
         {
            const BasicBlock *SrcBlock = SrcI->getParent();
            unsigned SrcLevel = LI.getLoopDepth(SrcBlock);
            if(SrcLevel==0)
              continue;
            else
            { 
              Value *SrcPtr = getLoadStorePointerOperand(&*SrcI);
              const SCEV *SrcSCEV = SE.getSCEV(SrcPtr);
              const SCEVAddRecExpr *AddRec = dyn_cast<SCEVAddRecExpr>(SrcSCEV);
              const SCEV *Step = AddRec->getStepRecurrence(SE);
              const SCEVConstant *stride=dyn_cast<SCEVConstant>(Step);
              int64_t stride_value = ((stride->getValue())->getSExtValue())/4;
              if(stride_value<=16)
              {
                errs() <<"  Loop Nest "<<LoopNest<<"\n";
                LoopNest++;
                break;
              }
            }
